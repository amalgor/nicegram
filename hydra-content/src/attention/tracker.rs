...
impl AttentionTracker {
    ...
    fn save(&self, data: &StoreData) -> Result<()> {
        let json = serde_json::to_string(data).context("Failed to serialize attention data")?;
        std::fs::write(&self.path, json).context("Failed to write attention tracker file")?;
        Ok(())
    }
    ...
    pub fn get_chat_stats(&self, chat_id: i64) -> Result<ChatAttentionStats> {
        let data = self
            ...
            .lock()
            .map_err(|e| anyhow::anyhow!("Lock poisoned: {}", e))?;
        let chat_events: Vec<&AttentionEvent> = data
            .events
            .iter()
        ...
        let avg_depth = if total > 0 {
            chat_events
                .iter()
                .map(|e| e.max_depth_reached as f32)
                .sum::<f32>()
                / total as f32
        } else {
            0.0
        };
        let deep_dives = chat_events
            .iter()
