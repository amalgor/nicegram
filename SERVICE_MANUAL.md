...
- **AI chat crash fix (2026-04-12)**: on-device chat crash after `Generating...` was caused by sampler misuse in `hydra-ai/src/models/qwen2_infer.rs`. The generation loop passed the absolute token position (`n_cur - 1`) into `LlamaSampler::sample()`, but llama.cpp expects an output-row index from the last `decode()`. In Hydra's batching pattern only the last decoded token has logits enabled, so sampling must use `-1` ("last output"). The same path also had an older double-accept bug (`sample()` already accepts internally). Runtime now samples from the last decode output without double-accept.
- **Manual regression command**: `cargo test -p hydra-ai ignored_short_prompt_smoke_test -- --ignored --nocapture` loads the smallest bundled GGUF (`hydra_mobile/assets/models/qwen2.5-0.5b.gguf`) and exercises real `Qwen2Infer::generate()` without Android UI.
